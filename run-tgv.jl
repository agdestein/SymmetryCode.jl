    ...
    for tgv in S.tgv_runs()
        ...
        for Δf in case.filters_test
            ...
            if :plots in config.experiments
                ...
                S.plot_dissipation_bar(case, tgv, Δf, withref)
                S.plot_error_post(case, tgv, Δf, models)
                S.plot_spectrum_les(case, tgv, Δf, withref)
            end
            :dissipation in config.experiments &&
                S.plot_dissipation_tgv(case, tgv, Δf, withref)
